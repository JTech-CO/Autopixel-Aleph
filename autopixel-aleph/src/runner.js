...
(() => {
  ...
  const rate = () => {
    ...
    return s > 0.25 ? state.paintedTotal / s : 0;
  };
  ...
  async function start(cells, profile, opts, userHooks) {
    ...
    let selection=null, revision=null, auditAt=0, auditCursor=0;
    const visited=new Map();
    const restart = nextRevision => {
      NS.engine.invalidateReuse();
      profile.sampleProof=null;
      revision=nextRevision;
      visited.clear();auditCursor=0;auditAt=performance.now();
      state.restarts++;
      // Counters and marks describe the new pass; speed counts all actual clicks.
      for(const key of ['done','blocked','matching','transparent','filtered','outside','deferred','unverified'])
        state[key]=0;
      NS.overlay.resetProgress();
      hooks.onProgress?.(state,null);
    };
    const changesTarget = (before,after) =>
      before.color!==after.color ||
      (before.kind!==after.kind && !(before.kind==='paint' && after.kind==='matching'));
    const audit = async final => {
      if(!native || !visited.size) return false;
      const entries=[...visited.entries()];
      let checked=0;
      do {
        const batch=entries.slice(auditCursor,auditCursor+128);
        const result=await NS.native.inspect(batch.map(([index])=>cells[index]),profile.shouldAbort);
        if(!result.ok) {
          if(final && !profile.shouldAbort()) {
            state.error=result.reason;state.comparisonReason=result.reason;
          }
          return false;
        }
        if(result.revision!==revision || result.values.some((v,j)=>changesTarget(batch[j][1],v))) {
          restart(result.revision);return true;
        }
        result.values.forEach((v,j)=>visited.set(batch[j][0],v));
        checked+=batch.length;
        auditCursor=(auditCursor+batch.length)%entries.length;
        if(final && checked<entries.length) await NS.engine.sleep(0.1);
      } while(final && checked<entries.length && !profile.shouldAbort());
      auditAt=performance.now();return false;
    };
    ...
    let lastPaintAt=0;
    profile.beforePaint = async (reusing = false) => {
      const targetRate=profile.variableFill && reusing && profile.targetRate===25 ? 30 : profile.targetRate;
      profile.paintRate=targetRate;
      if(!targetRate || !lastPaintAt) return;
      while(!profile.shouldAbort()) {
        const period=1000/targetRate;
        const due=native && profile.useKey && targetRate===25
    ...
    try {
      ...
      for (let i = 0; i <= cells.length; i++) {
        ...
        if(native && (i===cells.length || performance.now()-auditAt>=250)) {
          if(await audit(i===cells.length)) {i=-1;continue;}
          if(state.error || state.stopRequested) break;
        ...
        if(native) {
          if(revision!==null && observed.revision!==revision) {
            restart(observed.revision);i=-1;continue;
          }
          revision=observed.revision;
          visited.set(i,observed);
        }
        ...
        if (native) {
          ...
          profile.templateRevision = observed.revision;
          profile.recheck = async () => {
            const value = await NS.native.read(cell, profile.shouldAbort);
            if(value.ok && (value.revision!==revision || changesTarget(observed,value))) {
        ...
        if (result === 'ok') {
          ...
          if(native && profile.useKey && profile.paintRate===25) {
      ...
      if (state.paintedTotal && comparison?.mode === 'snapshot') NS.matching.invalidate();
      hooks.onEnd?.(state);
    }
  }
