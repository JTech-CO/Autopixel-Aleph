...
(() => {
  ...
  const TX = {
    ...
    lbl_variable_fill: { en: 'Variable fill', ko: '가변 채우기' },
    tip_variable_fill: { en: 'Reuse a confirmed color on matching template cells. Color changes are sampled again. Native Overlay only.', ko: '같은 도안 색에서는 확인한 선택색을 재사용하고, 색이 바뀌면 다시 추출합니다. 공식 자동 오버레이 전용입니다.' },
    'compare_native-variable': { en: 'Official overlay: variable fill', ko: '공식 도안: 가변 채우기' },
    sp_turbo_variable: { en: 'Turbo 25~30', ko: '터보 25~30' },
    ...
    tip_source:      { en: 'Overlay: pick template colors. Variable fill reuses confirmed colors. Current: paint with the color already selected in the palette.',
                       ko: '오버레이: 도안 색을 추출하며 가변 채우기를 켜면 확인한 색을 재사용. 현재색: 팔레트에 이미 선택된 색으로 채움.' },
