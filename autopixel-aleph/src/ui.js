...
${seg('selectionOp', ['replace', 'add', 'subtract'])}
        ...
        <section class="group">
          ...
          <label class="check" id="variableFillRow"><input type="checkbox" id="variableFill"><span data-lbl="lbl_variable_fill"></span></label>
  ...
  const els = {
    ...
    speed: $('speed'), source: $('source'), variableFill: $('variableFill'), order: $('order'),
    frames: $('frames'),
  ...
  function applyText() {
    ...
    for (const [group, map] of Object.entries(SEG_LABELS)) {
      for (const btn of els[group].querySelectorAll('button')) {
        btn.textContent = T(group==='speed' && btn.dataset.v==='turbo' && NS.store.cfg.comparisonMode==='native' && NS.store.cfg.source==='overlay' ? (NS.store.cfg.variableFill ? 'sp_turbo_variable' : 'sp_turbo_overlay') : map[btn.dataset.v]);
    ...
    els.variableFill.parentElement.title = T('tip_variable_fill');
  ...
  function render() {
    ...
    turbo.textContent=T(cfg.comparisonMode==='native' && cfg.source==='overlay'?(cfg.variableFill?'sp_turbo_variable':'sp_turbo_overlay'):'sp_turbo');
    ...
    els.variableFill.checked = cfg.variableFill;
    $('variableFillRow').hidden = cfg.comparisonMode !== 'native' || cfg.source !== 'overlay';
    els.variableFill.disabled = NS.runner.state.running || NS.pick.mode !== 'none';
    ...
    els.compareState.textContent = cfg.comparisonMode === 'native'
      ? T(cfg.source === 'overlay' ? (cfg.variableFill ? 'compare_native-variable' : 'compare_native-ready') : 'compare_native-current')
  ...
  els.variableFill.addEventListener('change', () => {
    if(NS.runner.state.running || NS.pick.mode !== 'none') return;
    NS.store.set({ variableFill: els.variableFill.checked });
  });
