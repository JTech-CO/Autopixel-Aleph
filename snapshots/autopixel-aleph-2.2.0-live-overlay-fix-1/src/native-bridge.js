...
(() => {
  ...
  // Track source identity and placement, not progress objects, which also change
  // after our own paint clicks. Coordinate values are copied to detect moves.
  function templateRevision(s) {
    let parts, progress;
    if(s.kind==='alliance') {
      const art=s.canvas.querySelector('.artboard-frame');
      parts=[art];
      for(const overlay of art?.querySelectorAll('.alliance-template-overlay') || []) {
        const canvas=overlay.querySelector('canvas'), source=allianceSources.get(overlay);
        const r=overlay.getBoundingClientRect(),style=canvas && getComputedStyle(canvas);
        parts.push(overlay,source?.data,source?.width,source?.height,
          r.left,r.top,r.width,r.height,canvas?.hidden,style?.display,Number(style?.opacity)>0);
      }
    } else {
      const layer=visible(s), d=layer.data;
      progress=d.statusHighlights?.progress;
      parts=[layer,d.pixels.data,d.pixels.width,d.pixels.height,...d.coordinates.flat()];
    }
    if(!s.templateParts || parts.length!==s.templateParts.length ||
        parts.some((v,i)=>v!==s.templateParts[i])) {
      if(s.templateParts && s.lastProgress) s.replacedProgress=s.lastProgress;
      s.templateParts=parts;s.revision=(s.revision||0)+1;
    }
    if(progress) s.lastProgress=progress;
    return s.revision;
  }
  function readCell(s,req) {
    if(!Number.isFinite(req.x)||!Number.isFinite(req.y)) throw Error('native-unsupported');
    if(s.kind==='alliance') return allianceRead(s,req);
    const layer = visible(s), rect = s.canvas.getBoundingClientRect();
    if (req.x < rect.left || req.y < rect.top || req.x >= rect.right || req.y >= rect.bottom)
      throw Error('native-canvas');
    const lngLat = s.map.unproject([req.x-rect.left, req.y-rect.top]);
    const { x, y, topLeft } = locate(layer, lngLat);
    const pixels = layer.data.pixels;
    if (x < 0 || y < 0 || x >= pixels.width || y >= pixels.height) {
      return { ok: true, kind: 'outside' };
    }
    const i = (y*pixels.width+x)*4, rgba = Array.from(pixels.data.subarray(i,i+4));
    if (rgba.length !== 4) throw Error('native-unsupported');
    if (rgba[3] < 16) { return { ok: true, kind: 'transparent' }; }
    ...
    if (!progress || progress===s.replacedProgress || progress.width !== pixels.width || progress.height !== pixels.height ||
    ...
    return { ok: true, kind: status === 1 ? 'matching' : 'paint',
  ...
  document.addEventListener('apx:native-request', async event => {
    ...
    try {
      ...
      if (!['read','inspect'].includes(req.op)) throw Error('native-unsupported');
      const revision=templateRevision(s);
      if(req.op==='inspect') {
        if(!Array.isArray(req.cells) || req.cells.length>128) throw Error('native-unsupported');
        const values=req.cells.map(cell=>readCell(s,cell));
        reply(req.id,{ok:true,revision,values});return;
      }
      reply(req.id,{...readCell(s,req),revision});
    } catch (e) {
