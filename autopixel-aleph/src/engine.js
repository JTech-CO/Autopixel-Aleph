...
(() => {
  ...
  let reuseEpoch = 0;
  const invalidateReuse = () => { reuseEpoch++; };
  const verification = {phase:'idle',expected:null,selected:null,attempt:0,reused:0,sampled:0};
  async function paintVerified(target, x, y, p) {
    ...
    const correct=()=>{
      ...
      return !!button?.getClientRects().length && (alliance
        ? !picking() && button.getAttribute('aria-pressed')==='true' && selected().length===1
    ...
    const canReuse = () => p.variableFill && p.sampleProof &&
      p.sampleProof.epoch===reuseEpoch && p.sampleProof.target===target &&
      p.sampleProof.revision===p.templateRevision && p.sampleProof.kind===p.nativeKind &&
      p.sampleProof.color===p.expectedColor && p.sampleProof.rgba===p.expectedRGBA?.join(',') && correct();
    if(canReuse()) {
      verification.phase='reuse';
      await p.beforePaint?.(true);
      const final=await p.recheck?.();
      if(p.shouldAbort?.()) return 'cancelled';
      if(final?.kind==='changed') {p.sampleProof=null;return 'changed';}
      if(final?.kind==='matching') return 'matching';
      if(!final?.ok) return fail(final?.reason || 'native-loading');
      if(final.kind!=='paint' || final.color!==p.expectedColor) {p.sampleProof=null;return 'deferred';}
      if(canReuse()) {
      ...
      // The palette changed while pacing. Re-sample at the normal rate.
    }
    p.sampleProof=null;
    if(!p.useKey) {
      ...
      const final=await p.recheck?.();
      if(p.shouldAbort?.()) return 'cancelled';
      if(final?.kind==='changed') return 'changed';
      if(final?.kind==='matching') return 'matching';
      if(!final?.ok) return fail(final?.reason || 'native-loading');
    ...
    for(let attempt=1;attempt<=3;attempt++) {
      ...
      if(p.shouldAbort?.()) return 'cancelled';
      if(p.variableFill && attemptEpoch!==reuseEpoch) return 'cancelled';
      verification.selected=selected()[0]?.id || selected()[0]?.getAttribute('aria-label') || null;
      if(!correct()) {
        const latest=await p.recheck?.();
        if(latest?.kind==='changed') return 'changed';
        if(p.shouldAbort?.()) return 'cancelled';
      ...
      await click(target,x,y,0);
      if(p.variableFill) p.sampleProof={epoch:attemptEpoch,target,revision:p.templateRevision,
        kind:p.nativeKind,color:p.expectedColor,rgba:p.expectedRGBA.join(',')};
      verification.phase='done';
      return 'ok';
    }
  ...
  function profileFrom(cfg) {
    ...
    return {
      ...
      variableFill: cfg.variableFill === true && cfg.comparisonMode === 'native' && cfg.source === 'overlay',
  ...
  NS.engine = {
    ...
    invalidateReuse,
    timing,
    resetTiming() { timing.frameWaitMs = 0; timing.frameWaits = 0; verification.reused=0; verification.sampled=0; invalidateReuse(); },
    frames,
    sleep,
    pickTarget,
    rememberTarget,
